...
@login_required
def viewpo_M(request):
    polist = Purchase_Order.objects.all()
    context = {
    ...
    return render(request,'viewpo_M/viewpo_Mlist.html',context)
    
def showpo_M(request):
    ...
    if (Purchase_Order.objects.filter(purchase_order_id = newpurchase_order_id).exists()):
        po = Purchase_Order.objects.filter(purchase_order_id = newpurchase_order_id).values_list('purchase_order_id', 'quotation_id', 'finance_officer_id', 'purchase_order_created_date', 'delivery_date', 'purchase_order_status')
        ...
        return render(request,'viewpo_M/error.html')
    context = {
    ...
    return render(request,'viewpo_M/viewpo_M.html',context)